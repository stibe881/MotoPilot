  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      sub = await Location.watchPositionAsync(
        ...
        (pos) => {
          const { route, isNavigating, stepIndex, setStepIndex, recalculate } =
          ...
          if (offBy > OFF_ROUTE_M && Date.now() - lastRecalc.current > RECALC_COOLDOWN_MS) {
          ...
          const endCoord = endIdx != null ? route.coordinates[endIdx] : undefined;
          if (
            endCoord &&
